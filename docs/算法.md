...
```java
/**
 * @see <a href="https://leetcode.cn/problems/yong-liang-ge-zhan-shi-xian-dui-lie-lcof/">leetcode</a>
 */
class CQueue {
...
```java
/**
 * @see <a href="https://leetcode-cn.com/problems/add-two-numbers/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode-cn.com/problems/median-of-two-sorted-arrays/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode-cn.com/problems/reverse-integer/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode-cn.com/problems/remove-nth-node-from-end-of-list/">leetcode</a>
 */
public ListNode removeNthFromEnd(ListNode head, int n) {
    // 生成临时的头节点，解决需要删除头节点的情况
...
```java
/**
 * @see <a href="https://leetcode-cn.com/problems/merge-two-sorted-lists/">leetcode</a>
 */
...
/**
 * @see <a href="https://leetcode-cn.com/problems/next-permutation/">leetcode</a>
 */
public static void nextPermutation(int[] nums) {
    ...
    int i = nums.length - 2, j = nums.length - 1, k = j;
    ...
    // 反转 A[j:end]
    for (int begin = j, end = nums.length - 1; begin < end; begin++, end--) {
        swap(nums, begin, end);
    }
}
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/search-in-rotated-sorted-array/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/find-first-and-last-position-of-element-in-sorted-array/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/swap-nodes-in-pairs/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/combination-sum/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/rotate-image/solution/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/merge-intervals/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/minimum-path-sum/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/sort-colors/submissions/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/construct-binary-tree-from-preorder-and-inorder-traversal/">leetcode</a>
 */
static int p = 0;
static int i = 0;

public static TreeNode buildTree(int[] preorder, int[] inorder) {
    // 初始设置stop 为 Integer.MAX_VALUE + 1，是为了第一次判断为true
    return solute((long) Integer.MAX_VALUE + 1, preorder, inorder);
}

public static TreeNode solute(long stop, int[] pre, int[] in) {
    //利用中序遍历重构  也就是最左遍历
    //当stop==in[i] 此时stop对应节点为最左节点
...
private static TreeNode buildTreeHelper(int[] preorder, int p_start, int p_end,
    ...
    // 通过map获取 root_val 对应的下标
    int i_root_index = map.get(root_val);
    // 当前节点的左子节点个数 = 中序遍历下标索引 -  i_start
    int leftNum = i_root_index - i_start;
    // 递归构造左子树，前序遍历开始 + 1，前序遍历结束 + leftNum + 1，中序遍历开始不变，中序遍历结束为i_root_index
    root.left = buildTreeHelper(preorder, p_start + 1, p_start + leftNum + 1, inorder, i_start, i_root_index, map);
    // 递归构造右子树，前序遍历开始 + leftNum + 1，前序遍历结束不变，中序遍历开始i_root_index + 1，中序遍历结束不变
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/remove-duplicates-from-sorted-array/submissions/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/word-search/">leetcode</a>
 */
...
public static int longestConsecutive1(int[] nums) {
        int res = 0;
        // int[0]：小于等int[0]的连续序列个数，int[1]：大于等于int[1]的连续序列个数
        Map<Integer, int[]> map = new HashMap<>(nums.length);
        for (int num : nums) {
            if (!map.containsKey(num)) {
                int small = num - 1;
                int[] smallArr = map.get(small);
                int big = num + 1;
                int[] bigArr = map.get(big);
                // 小于等于 num 的个数
                int smallCnt = 1;
                if (smallArr != null) {
                    smallCnt = smallArr[0] + 1;
                    smallArr[1]++;
                    small--;
                    while (true) {
                        smallArr = map.get(small);
                        if (smallArr == null) {
                            break;
                        } else {
                            smallArr[1]++;
                ...
                // 更新res
                res = Math.max(res, bigCnt + smallCnt - 1);
                map.put(num, new int[]{smallCnt, bigCnt});
            }
        }
        return res;
    }
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/longest-consecutive-sequence/">leetcode</a>
 */
public int longestConsecutive(int[] nums) {
    ...
    for (int num : nums) {
        int small = num - 1;
        int big = num + 1;
        while (set.contains(small)) small--;
        while (set.contains(big)) big++;
...
public static int longestConsecutive1(int[] nums) {
    ...
    for (int num : nums) {
        if (!map.containsKey(num)) {
            int small = 1;
            int[] smallArr = map.get(num - small);
            int big = 1;
            int[] bigArr = map.get(num + big);
            // 小于等于 num 的个数
            int smallCnt = smallArr == null ? 1 : smallArr[0] + 1;
            // 大于等于 num 的个数
            int bigCnt = bigArr == null ? 1 : bigArr[1] + 1;
            // 向 small 更新
            while (true) {
                smallArr = map.get(num - small);
                if (smallArr == null) {
                    break;
                }
                smallArr[1] = bigCnt + small;
                small++;
            }
            // 向 big 更新
            while (true) {
                bigArr = map.get(num + big);
                if (bigArr == null) {
                    break;
                }
                bigArr[0] = smallCnt + big;
                big++;
            }
            // 更新res
            res = Math.max(res, bigCnt + smallCnt - 1);
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/single-number/">leetcode</a>
 */
 ...
 */
/**
 * @see <a href="https://leetcode.cn/problems/lru-cache/">leetcode</a>
 */
...
/**
 * @see <a href="https://leetcode.cn/problems/min-stack/">leetcode</a>
 */
class MinStack {
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/intersection-of-two-linked-lists/">leetcode</a>
 */
public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
    ...
    // 长短不同的两调想交的链表，先到达末端的哪个再从长的链表的头部开始遍历，
    // 后达到末端的再从短的链表头部开始遍历，最终走过相同距离会在第一个相交节点相遇
    while (nodeA != nodeB) {
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/majority-element/">leetcode</a>
 */
public int majorityElement(int[] nums) {
    Arrays.sort(nums);
    // 数组中间的元素总是“多数元素”，毕竟它长度 大于 n/2
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/number-of-islands/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/implement-trie-prefix-tree/">leetcode</a>
 */
class Trie {

    private class TrieNode {
        // 用于标记当前是否是某个插入字符的最后一个字符
        boolean isEnd;
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/kth-largest-element-in-an-array/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/course-schedule/">leetcode</a>
 */
public boolean canFinish(int numCourses, int[][] prerequisites) {
    ...
    for (int[] prerequisite : prerequisites) {
        ...
        // 想要学习prerequisite[0]，就必须先学习prerequisite[1]
        // 所以prerequisite[1]是prerequisite[0]的前置条件，放入链表
        adjacency.get(prerequisite[1]).add(prerequisite[0]);
    }

    // 将没有前置条件的课程放入队列
    for (int i = 0; i < numCourses; i++) {
        if (needs[i] == 0) queue.add(i);
    }
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/maximal-square/">leetcode</a>
 */
public static int maximalSquare(char[][] matrix) {
    int rows = matrix.length;
    int cols = matrix[0].length;

    // 最左边多初始一列 和 最上边多初始一行，用于动态规划
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/invert-binary-tree/">leetcode</a>
 */
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = m; j >= time; j--) {
                dp[j] = Math.max(dp[j], dp[j - time] + val);
            }
        }
...
```java
/**
 * @see <a href="https://leetcode-cn.com/problems/minimum-falling-path-sum/">leetcode</a>
 */
...
/**
 * @see <a href="https://leetcode-cn.com/problems/target-sum/">leetcode</a>
 */
